...
# A lock prevents the MJPEG generator from reading a half-written frame
# -------------------------
...
def _update_suggestions(word: str):
    ...
    if word.strip():
        suggs = sym_spell.lookup(
            word, symspellpy.Verbosity.CLOSEST, max_edit_distance=2
        )[:4]
    ...
    if ch1 in VALID_CHARS:
        ...
        if auto_type_char != " ":
            ...
            if words and len(words[-1]) > 1:
                suggs = sym_spell.lookup(
                    words[-1], symspellpy.Verbosity.CLOSEST, max_edit_distance=2
                )
                if suggs:
...
def _open_camera():
    ...
    # NOTE: do NOT set CAP_PROP_BUFFERSIZE=1 on Windows DSHOW — it causes the
    # driver to deliver partial/torn frames because the buffer is flushed before
    # the frame is fully written. Leave it at the default (usually 3-5 frames).
    if sys.platform != "win32":
...
def background_camera_loop():
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            sy = h / MP_HEIGHT
            abs_pts = [
                [int(lm.x * MP_WIDTH * sx), int(lm.y * MP_HEIGHT * sy)]
            ...
            for i, j in HAND_CONNECTIONS:
                cv2.line(
                    _display_white, tuple(abs_pts[i]), tuple(abs_pts[j]), (0, 0, 0), 2
                )
            for i in range(21):
        ...
        skel_bytes = buf_skel.tobytes()

        # Atomically swap both frame buffers under the lock so the MJPEG
        # generator never reads a camera frame paired with the wrong skeleton
        with _frame_lock:
            camera_frame = cam_bytes
            skeleton_frame = skel_bytes
...
def generate_feed(src="camera"):
    """Yield MJPEG frames, protected by a lock so we never read a partial write."""
    last_id = None
    while True:
        with _frame_lock:
            frame = camera_frame if src == "camera" else skeleton_frame
        # Use id() for identity check — comparing bytes content is too expensive
...
def video_feed():
    return StreamingResponse(
        generate_feed("camera"), media_type="multipart/x-mixed-replace; boundary=frame"
    )
...
def _autocorrect_sentence():
    ...
    for w in words:
        if len(w.strip()) > 1:
            suggs = sym_spell.lookup(
                w, symspellpy.Verbosity.CLOSEST, max_edit_distance=2
            )
